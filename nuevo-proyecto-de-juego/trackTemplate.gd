...
func _process(delta: float) -> void:
	#esta_en_curva(pos_x: float, margen: float = 50.0) -> bool:
	#if esta_en_curva()
	#obtener_nodo_por_id
	if estado_actual != EstadoCarrera.End:
		actualizar_posiciones()
		#print(order)
		for id_corredora in order: #order es por ids no por nodos
			#var faltante: float = distanciaRestante(id_corredora)
			var nodo_corredora: Node2D = obtener_nodo_por_id(id_corredora)
			if esta_en_curva(nodo_corredora.global_position.x):
				pass
			...
			if esta_en_bajada(nodo_corredora.global_position.x):	
				nodo_corredora.decrease_stamina_usage(1.5) 
			else:
				nodo_corredora.set_normal_stamina_usage()
			print(nodo_corredora.global_position.x)
			#var nodo_corredora: Node2D = obtener_nodo_por_id(id_corredora)
			
			#if nodo_corredora:
			#	var nombre: String = nodo_corredora.get_nombre()
			#	print(str(faltante) + " m restantes de " + nombre)
...
func esta_en_bajada(pos_x: float) -> bool:
	for rango in bajadas:
		if pos_x >= rango.x and pos_x <= rango.y:
			print("entro a una bajada")
			return true
	return false

func esta_en_curva(pos_x: float, margen: float = 50.0) -> bool:
	for rango in curvas:
		print(str(rango.x) +"-"+ str(rango.y) +"y esta en" +str(pos_x))
		if pos_x >= rango.x and pos_x <= rango.y:
			print("entro a una curva")
			return true
	return false
