...
func cargar_participantes(escenas_corredoras: Array[PackedScene]) -> void:
	race_start.cargar_participantes(self, escenas_corredoras)

func antesDeIniciar() -> void:
	race_start.antesDeIniciar(self)

func generar_corredoras() -> void:
	race_start.generar_corredoras(self)
#==============Fase Inicial====================
...
func obtener_nodo_por_id(id_buscado: int) -> Node2D:
	for hijo in get_children():
		if hijo.has_method("get_id") and hijo.get_id() == id_buscado:
			return hijo
	return null
